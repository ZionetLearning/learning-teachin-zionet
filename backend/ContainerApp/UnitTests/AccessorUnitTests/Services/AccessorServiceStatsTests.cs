...
public class AccessorServiceStatsTests
{
    ...
    public async Task ComputeStatsAsync_Computes_All_Counters_With_Time_Windows()
    {
        ...
        var t1 = new ChatThread { ThreadId = Guid.NewGuid(), UserId = "u1", ChatType = "default", CreatedAt = now.AddHours(-2), UpdatedAt = now.AddHours(-1) };
        var t2 = new ChatThread { ThreadId = Guid.NewGuid(), UserId = "u2", ChatType = "default", CreatedAt = now.AddHours(-2), UpdatedAt = now.AddMinutes(-20) };
        var t3 = new ChatThread { ThreadId = Guid.NewGuid(), UserId = "u2", ChatType = "default", CreatedAt = now.AddHours(-3), UpdatedAt = now.AddMinutes(-1) };
        db.ChatThreads.AddRange(t1, t2, t3);
