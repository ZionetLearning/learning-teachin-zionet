...
) : AiChatTestBase(sharedFixture.HttpFixture, outputHelper, signalRFixture), IAsyncLifetime
{
    ...
    [InlineData(Difficulty.medium, true)]
    [InlineData(Difficulty.hard, false)]
    public async Task MistakeExplanation_DifferentDifficulties_ShouldWork(Difficulty difficulty, bool nikud)
    {
        // Arrange
        var user = _shared.UserFixture.TestUser;
        await _shared.GetAuthenticatedTokenAsync();
        ...
        OutputHelper.WriteLine($"Testing mistake explanation for difficulty: {difficulty}, nikud: {nikud}");
        ...
        frames.Should().NotBeNull();
        frames.Length.Should().BeGreaterThan(0);
        ...
        OutputHelper.WriteLine($"Explanation for {difficulty}/{nikud}: {explanation.Substring(0, Math.Min(100, explanation.Length))}...");
    }
}