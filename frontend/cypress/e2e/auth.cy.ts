describe('auth', () => {
	...
	it('invalid login shows validation or stays on form', () => {
		...
		cy.get('[data-testid="auth-password"]')
			...
			.type('badpass');
		cy.get('[data-testid="auth-submit"]').click();
		// App currently auto "logs in" via login() call; if invalid handling added later, adjust assertion.
		// For now, ensure we either still see the form or got sidebar (tolerant until real backend).
		cy.get('body').then(($b) => {
