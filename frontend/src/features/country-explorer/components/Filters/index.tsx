...
export type FiltersState = {
    search: string;
    region: 'All' | 'Africa' | 'Americas' | 'Asia' | 'Europe' | 'Oceania' | 'Antarctic';
...
export const Filters: React.FC<Props> = ({ value, onChange }) => {
    ...
    return (
        ...
        >
            <div>
                <label className={classes.label}>{t('pages.countryExplorer.searchByName')}</label>
                <input
            ...
            <div>
                <label className={classes.label}>{t('pages.countryExplorer.regionWithNoColon')}</label>
                <select
                    className={classes.select}
                    value={value.region}
                    onChange={(e) => update('region', e.target.value as FiltersState['region'])}
                >
            ...
            <div>
                <label style={{ display: 'block', fontSize: 12, color: '#555' }}>{t('pages.countryExplorer.populationWithNoColon')}</label>
                <select
                    className={classes.select}
                    value={value.popRange}
                    onChange={(e) => update('popRange', e.target.value as PopRangeKey)}
                >
                    {popRanges.map(p => <option key={p.key} value={p.key}>{p.label}</option>)}
                </select>
            </div>
        </div>
    );
