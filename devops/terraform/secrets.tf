...
resource "kubernetes_secret" "postgres_connection" {
  ...
  data = {
    PostgreSQLConnectionString = var.use_shared_postgres ? format("Host=%s;Database=%s;Username=%s;Password=%s;SslMode=Require", data.azurerm_postgresql_flexible_server.shared[0].fqdn, "${var.environment_name}-${var.database_name}", var.admin_username, var.admin_password) : module.database[0].postgres_connection_string
  }
}
