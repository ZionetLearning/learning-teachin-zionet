...
# Frontend outputs (conditional based on enable_static_web_apps flag)
output "static_web_app_urls" {
  description = "URLs of the Azure Static Web Apps"
  value       = var.enable_static_web_apps ? { for app_name, frontend in module.frontend : app_name => frontend.static_web_app_url } : {}
}

output "static_web_app_api_keys" {
  description = "API keys for the Azure Static Web Apps"
  value       = var.enable_static_web_apps ? { for app_name, frontend in module.frontend : app_name => frontend.static_web_app_api_key } : {}
  sensitive   = true
}

output "application_insights_connection_strings" {
  description = "Connection strings for Application Insights per frontend app"
  value       = var.enable_static_web_apps ? { for app_name, frontend in module.frontend : app_name => frontend.application_insights_connection_string } : {}
  sensitive   = true
}

output "enable_static_web_apps" {
  description = "Whether Static Web Apps are enabled"
  value       = var.enable_static_web_apps
}
