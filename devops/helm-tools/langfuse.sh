...
helm $ACTION langfuse langfuse/langfuse \
  ...
  --set-string langfuse.additionalEnv[35].value="You have been granted access to TeachIn Langfuse" \
  --set langfuse.additionalEnv[36].name="MEMBERSHIP_INVITATION_EMAIL_TEMPLATE" \
  --set-string langfuse.additionalEnv[36].value="<div style=\"font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;\"><div style=\"background: #667eea; color: white; padding: 20px; text-align: center;\"><h1>Welcome to TeachIn Langfuse</h1></div><div style=\"background: #f9f9f9; padding: 30px;\"><p>Hello,</p><p><strong>{{inviterName}}</strong> has granted you access to <strong>{{organizationName}}</strong> on Langfuse.</p><div style=\"background: white; padding: 20px; border-left: 4px solid #667eea; margin: 20px 0;\"><p><strong>Role:</strong> {{role}}</p><p><strong>Organization:</strong> {{organizationName}}</p></div><p><a href=\"{{url}}\" style=\"background: #667eea; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;\">Accept Invitation</a></p><p><em>Note: You need to create an account with this email address if you do not have one yet.</em></p><p>Or copy this URL: {{url}}</p></div></div>" \
