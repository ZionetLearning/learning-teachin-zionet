...
helm $ACTION langfuse langfuse/langfuse \
  ...
  --set langfuse.additionalEnv[23].name="MEMBERSHIP_INVITATION_EMAIL_SUBJECT" \
  --set-string langfuse.additionalEnv[23].value="You're invited to join TeachIn's Langfuse" \
  --set langfuse.additionalEnv[24].name="MEMBERSHIP_INVITATION_EMAIL_TEMPLATE" \
  --set-string langfuse.additionalEnv[24].value="You have been invited to join TeachIn's Langfuse instance. Click the link below to accept the invitation." \
  --set redis.auth.existingSecret="langfuse-secrets" \
  --set redis.auth.existingSecretPasswordKey="REDIS_PASSWORD" \
